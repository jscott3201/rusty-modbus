...
use modbus_frame::frame::FrameHeader;
use modbus_rtu::rtu_tcp::RtuOverTcpTransport;
use modbus_tcp::config::TcpServerConfig;
use modbus_tcp::listener::TcpServerListener;
use modbus_tcp::transport::{TransportSink, TransportStream};
use modbus_tcp::TcpConfig;
use modbus_types::{ExceptionCode, TransactionId, UnitId};
...
impl ModbusGateway {
    ...
    pub async fn start(config: GatewayConfig) -> Result<Self, GatewayError> {
        ...
        let listener = TcpServerListener::bind(config.tcp_listen, tcp_config)
            .await
            .map_err(|e| match e {
                modbus_tcp::TransportError::Io(io) => GatewayError::Bind(io),
...
async fn handle_tcp_connection(
    mut tcp_sink: modbus_tcp::TcpSink,
    mut tcp_stream: modbus_tcp::TcpRecvStream,
    route_table: Arc<RouteTable>,
    serial_timeout: std::time::Duration,
) {
