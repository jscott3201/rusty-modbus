...
use modbus_frame::error::FrameError;
...
impl modbus_tcp::TransportSink for TlsSink {
    async fn send(&mut self, frame: Frame) -> Result<(), modbus_tcp::TransportError> {
        let fut = SinkExt::send(&mut self.inner, frame);
        if let Some(dur) = self.write_timeout {
            timeout(dur, fut)
                .await
                .map_err(|_| modbus_tcp::TransportError::Timeout)?
                .map_err(modbus_tcp::TransportError::Frame)
        } else {
            fut.await.map_err(modbus_tcp::TransportError::Frame)
        }
    }
}
...
impl modbus_tcp::TransportStream for TlsRecvStream {
    async fn recv(&mut self) -> Result<Frame, modbus_tcp::TransportError> {
        ...
        match item {
            ...
            Some(Err(e)) => Err(modbus_tcp::TransportError::Frame(e)),
            None => Err(modbus_tcp::TransportError::Disconnected),
        }
    }
}
