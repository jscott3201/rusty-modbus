...
fn spec_6_19_mei_request_decode() {
    ...
    match decode_request(&pdu).unwrap() {
        rusty_modbus_codec::RequestPdu::EncapsulatedInterface(r) => {
            assert_eq!(
                r.mei_type,
...
fn spec_6_21_device_id_request_encode() {
    ...
    use rusty_modbus_types::DeviceIdCode;
...
fn spec_6_21_device_id_response_decode() {
    ...
    let data: &[u8] = &[
        ...
        // Object 0: VendorName
        0x00, 0x05, b'T', b'e', b's', b't', b'!', // Object 1: ProductCode
        0x01, 0x03, b'X', b'Y', b'Z', // Object 2: Revision
        0x02, 0x05, b'1', b'.', b'0', b'.', b'0',
    ];
    let resp = ReadDeviceIdentificationResponse::decode(data).unwrap();
    assert_eq!(
        resp.device_id_code,
        rusty_modbus_types::DeviceIdCode::BasicStream
    );
...
fn spec_6_21_more_follows() {
    ...
    let data: &[u8] = &[
        0x0E, 0x01, 0x01, 0xFF, // More follows = YES
