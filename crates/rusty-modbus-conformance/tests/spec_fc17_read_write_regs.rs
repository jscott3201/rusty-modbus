...
const SPEC_REQUEST: &[u8] = &[
    0x17, 0x00, 0x03, // read address = 3 (register 4, 0-indexed)
...
const SPEC_RESPONSE: &[u8] = &[
    0x17, 0x0C, // byte count = 12 (6 registers × 2 bytes)
...
fn spec_6_17_request_decode() {
    match decode_request(SPEC_REQUEST).unwrap() {
        rusty_modbus_codec::RequestPdu::ReadWriteMultipleRegisters(r) => {
...
fn spec_6_17_response_decode() {
    match decode_response(SPEC_RESPONSE).unwrap() {
        rusty_modbus_codec::ResponsePdu::ReadWriteMultipleRegisters(r) => {
...
fn quantity_boundaries() {
    ...
    let zero_read = [
        0x17, 0x00, 0x00, 0x00, 0x00, // read addr=0, qty=0
        0x00, 0x00, 0x00, 0x01, 0x02, 0x00, 0x01, // write addr=0, qty=1, bc=2, data
    ];
    assert!(matches!(
        decode_request(&zero_read),
