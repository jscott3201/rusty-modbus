...
use modbus_codec::request::{Encode, ReadFifoQueueRequest};
use modbus_frame::OwnedResponsePdu;
use modbus_types::{Address, FunctionCode, UnitId};
...
impl ModbusClient {
    ...
    pub async fn read_fifo_queue(
        ...
        let len = req.encode_into(&mut buf).map_err(|_| ClientError::Codec(
            modbus_codec::DecodeError::Truncated { expected: 3, actual: 0 },
        ))?;
