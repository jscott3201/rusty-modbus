...
use modbus_frame::OwnedDeviceIdentification;
use modbus_frame::OwnedResponsePdu;
use modbus_types::{DeviceIdCode, FunctionCode, UnitId};
    ...
    pub async fn read_device_identification(
        ...
        loop {
            ...
            let len = req.encode_into(&mut buf).map_err(|_| {
                ClientError::Codec(modbus_codec::DecodeError::Truncated {
                    expected: 4,
                    actual: 0,
                })
            })?;
            ...
            let pdu_data = match response {
                ...
                _ => {
                    return Err(ClientError::Codec(
                        modbus_codec::DecodeError::UnknownFunctionCode(0),
                    ))
                }
            };
