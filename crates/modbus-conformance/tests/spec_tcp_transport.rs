...
use modbus_tcp::config::{AccessControl, AccessMode, TcpConfig, TcpServerConfig};
use modbus_tcp::listener::TcpServerListener;
use modbus_tcp::transport::{TransportConnect, TransportSink, TransportStream};
use modbus_tcp::TcpTransport;
...
fn spec_4_4_1_2_max_client_transactions() {
    ...
    assert_eq!(modbus_types::MAX_CLIENT_TRANSACTIONS, 16);
}
...
async fn transport_disconnect_detected() {
    ...
    assert!(matches!(
        stream.recv().await,
        Err(modbus_tcp::TransportError::Disconnected)
    ));
}
