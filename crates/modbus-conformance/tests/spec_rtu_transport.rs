...
use modbus_rtu::config::{DataBits, Parity, RtuConfig, StopBits};
...
async fn rtu_over_tcp_round_trip() {
    ...
    let config = modbus_tcp::config::TcpConfig::default();
    let (mut sink, mut stream) =
        modbus_rtu::rtu_tcp::RtuOverTcpTransport::connect(addr, config)
            .await
            .unwrap();
