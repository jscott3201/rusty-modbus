...
fn rtu_decode_fc03_request() {
    ...
    let mut codec = modbus_frame::rtu::RtuCodec;
...
fn rtu_bad_crc_error() {
    ...
    assert!(matches!(
        ...
        Err(modbus_frame::FrameError::CrcMismatch { .. })
    ));
}
...
fn rtu_tcp_decode_with_crc_scanning() {
    ...
    let mut buf = BytesMut::from(&raw[..]);
    let mut codec = modbus_frame::rtu_tcp::RtuOverTcpCodec;
...
fn rtu_tcp_overflow_detection() {
    ...
    assert!(matches!(
        codec.decode(&mut buf),
        Err(modbus_frame::FrameError::Truncated)
    ));
}
