...
use modbus_codec::request::{Encode, ReadHoldingRegistersRequest};
...
fn spec_6_3_request_decode() {
    match decode_request(SPEC_REQUEST).unwrap() {
        modbus_codec::RequestPdu::ReadHoldingRegisters(r) => {
...
fn spec_6_3_response_decode() {
    match decode_response(SPEC_RESPONSE).unwrap() {
        modbus_codec::ResponsePdu::ReadHoldingRegisters(r) => {
