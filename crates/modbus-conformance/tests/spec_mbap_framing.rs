...
use modbus_types::{MbapHeader, MBAP_HEADER_LEN, MAX_PDU_SIZE};
...
fn invalid_protocol_id_rejected() {
    ...
    assert!(matches!(
        ...
        Err(modbus_frame::FrameError::InvalidProtocolId(1))
    ));
}
...
fn length_overflow_rejected() {
    ...
    assert!(matches!(
        codec.decode(&mut buf),
        Err(modbus_frame::FrameError::LengthOverflow(_))
    ));
}
