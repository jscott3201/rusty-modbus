...
//!
//! These mirror the borrowed response types in `modbus_codec::response` but hold
...
use bytes::Bytes;
use modbus_codec::error::DecodeError;
...
use modbus_types::{DiagnosticSubFunction, FunctionCode, MeiType};
...
/// responses are used directly from `modbus_codec::response`.
#[derive(Debug)]
