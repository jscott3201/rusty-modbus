...
| [`CONF-001`](#requirement-conf-001) | Public function and profile claims trace to implementation and tests | `supported` | `implemented` | This seed maps implementation and tests but records no executed-test evidence. |
| [`CONF-002`](#requirement-conf-002) | Boundary and exception behavior has direct tests and mechanical requirement coverage | `compatibility-deviation` | `implemented` | The test suite has direct examples, but requirement-to-test completeness remains a follow-up. |
| [`CONF-003`](#requirement-conf-003) | Parsers have malformed-input and arbitrary stream-chunking evidence | `compatibility-deviation` | `implemented` | Complete-candidate RTU byte fuzzing is present; physical timing/event-state fuzzing remains assigned to PR-102. |
...
| `RTU-004` — At least t3.5 of silence separates RTU frames | `MUST` | [modbus-serial-line](https://www.modbus.org/file/secure/modbusoverserial.pdf) `V1.02`, §2.5.1.1, RTU transmission mode timing | `crates/rusty-modbus-frame/src/rtu.rs`; `crates/rusty-modbus-rtu/src/config.rs`; `crates/rusty-modbus-rtu/src/serial.rs` | `spec_rtu_transport`; Gap: Silence handling relies on client delays and does not provide complete receiver-boundary proof. (PR-102, PR-103) |
<a id="requirement-rtu-005"></a>
| `RTU-005` — An inter-character gap greater than t1.5 invalidates a partial RTU frame | `MUST` | [modbus-serial-line](https://www.modbus.org/file/secure/modbusoverserial.pdf) `V1.02`, §2.5.1.1, frame reception state diagram | `crates/rusty-modbus-frame/src/rtu.rs`; `crates/rusty-modbus-rtu/src/config.rs`; `crates/rusty-modbus-rtu/src/serial.rs` | Gap: The receive path does not invalidate partial frames specifically at t1.5. (PR-102) |
...
| `RTU-009` — Noise, bad CRCs, and overlength frames are discarded without preventing later valid frames | `MUST` | [modbus-serial-line](https://www.modbus.org/file/secure/modbusoverserial.pdf) `V1.02`, §2.5.1.1 and §2.5.1.2, frame analysis and CRC checking | `crates/rusty-modbus-frame/src/rtu.rs`; `crates/rusty-modbus-rtu/src/config.rs`; `crates/rusty-modbus-rtu/src/serial.rs` | `spec_rtu_framing`; Gap: Bad-CRC recovery exists; noise, overlength, and post-error recovery need broader evidence. (PR-103) |
...
| `CONF-003` — Parsers have malformed-input and arbitrary stream-chunking evidence | `project-profile` | [rusty-modbus-project-policy](schema.md) `1`, § Test inventory, hostile and malformed input evidence | `crates/rusty-modbus-codec/src/decode.rs`; `crates/rusty-modbus-frame/src/mbap.rs`; `crates/rusty-modbus-frame/src/rtu.rs`; `fuzz/fuzz_targets/pdu_decode.rs`; `fuzz/fuzz_targets/mbap_stream.rs`; `fuzz/fuzz_targets/rtu_frame.rs`; `fuzz/fuzz_targets/rtu_tcp_stream.rs`; `fuzz/fuzz_targets/support.rs`; `fuzz/corpus/manifest.json`; `scripts/fuzz.py`; `.github/workflows/fuzz.yml` | `spec_encode_validation`, `spec_mbap_framing`, `spec_parser_resilience`, `spec_rtu_framing`; Gap: The byte-parser and stream-framer fuzz foundation is present; physical RTU timing/event-state fuzzing and RTU-over-TCP boundary/recovery policy remain assigned to separate packages. (PR-102, PR-104) |
...
| `F-024` | `P2` | Evidence gap | `closed` | maintainers | `PR-003` | `CONF-003` | Parser property and fuzz evidence is incomplete | Closed for the byte-parser and stream-framer foundation by fixed-seed properties, four pinned cargo-fuzz targets, a retained manifest corpus, deterministic replay, and bounded scheduling. Physical RTU timing/event state remains assigned to PR-102, and RTU-over-TCP boundary/recovery policy remains assigned to PR-104. |
